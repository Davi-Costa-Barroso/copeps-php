						...
						<div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">Contact
										...
										<div class="row">
												<div class="col-md-8">							
														...
														<input type="text" class="form-control" id="nome_coordenador" name="nome_coordenador" placeholder="Nome do Coordenador" required>							
												</div>

												<div class="col-md-4">
												    <label for="sexoCoordenador">Sexo do Coordenador:</label>
												    <select class="form-control" id="sexoCoordenador" name="sexoCoordenador" required>
              ...
              <!-- Comecça daqui pra analisarrrrrrrrrrrrrrrrrrrrrrrrrrrrrr -->
								...
								<div class="row" id="outroCoordenadorInfo" style="display: none;">
												<div class="col-md-6">							
														...
														<input type="text" class="form-control" id="nomeOutroCoordenador" name="nomeOutroCoordenador" placeholder="Nome do Outro Coordenador" required>							
												</div>
	    ...
	    $.ajax({
	        ...
	        success: function(result) {
	            ...
	            for (var i = 0; i < result.length; i++) {
	                select.append($('<option>', { value: result[i], text: result[i] }));
	            }
	            ...
	            select.change(function() {
	                ...
	                if (escolhidoSegundoSelect === "Alocação de Carga horária") {
	                    ...
	                    terceiroSelect.find('#tipo').append($('<option>', { value: "Projeto de Ensino", text: "Projeto de Ensino" }));
	                    terceiroSelect.find('#tipo').append($('<option>', { value: "Projeto de Pesquisa", text: "Projeto de Pesquisa" }));
	                    terceiroSelect.find('#tipo').append($('<option>', { value: "Projeto de Extensão", text: "Projeto de Extensão" }));
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (escolhido2 === 'naoc') {
                ...
                $('#tipo-carga').append($('<option>', { value: 'desabilitado', text: 'Sem Alocação de Carga Horária', selected: 'selected' }));
            } else {
...
function contarCheckbox10() {
    ...
    $.ajax({
        ...
        method: 'POST',
        data: { escolhido3: totalSelecionados },
        dataType: 'json',
        success: function(result) {
            if (result.success) {
...
function contarCheckbox20() {
    ...
    $.ajax({
        ...
        method: 'POST',
        data: { escolhido4: totalSelecionados2 },
        dataType: 'json',
        success: function(result) {
            if (result.success) {
                // Faça algo com a resposta do servidor, se necessário
...
<script type="text/javascript">
	$("#seguinte_aba2").click(function () {
		...
		else if($("#obs1").val() == "" || $("#obs1").val() == null){
			$('#mensagem').addClass('text-danger')
			$('#mensagem').text("Preencha o Texto enviado do item do Ofício!")
		}
		else if($("#obs2").val() == "" || $("#obs2").val() == null){
			$('#mensagem').addClass('text-danger')
			$('#mensagem').text("Preencha o Campo Título do Projeto!")
		}
		else if($("#obs3").val() == "" || $("#obs3").val() == null){
			$('#mensagem').addClass('text-danger')
	...
	$("#seguinte_aba3").click(function () {
		if($("#relatorio-div").val() == "" || $("#relatorio-div").val() == null){
			$('#mensagem').addClass('text-danger')
			$('#mensagem').text("Preencha o Campo Relatório! ")
		}
		else if($("#mesesSelect").val() == "" || $("#mesesSelect").val() == null){
			$('#mensagem').addClass('text-danger')
			$('#mensagem').text("Selecione o Tempo vigência do Relatório!")
		}
		else if($("#carga-horaria-div").val() == "" || $("#carga-horaria-div").val() == null){
			$('#mensagem').addClass('text-danger')
...
<!-- Script pra pegar somente o nicedit na area desejada -->
